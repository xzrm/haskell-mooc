...
instance Foldable List where
  foldr _ initValue Empty = initValue
  foldr f initValue (LNode x xs) = f x (foldr f initValue xs)
...
instance Foldable TwoList where
  foldr f i TwoEmpty = i
  foldr f i (TwoNode x1 x2 xs) = f x1 (f x2 (foldr f i xs))
