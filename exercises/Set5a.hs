...
setName newName (PersonUndefined age name) = PersonUndefined age newName
...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix c1 c2) = [uncurry average i | i <- zip (rgb c1) (rgb c2)]
  where
...
fromNat :: Nat -> Int
fromNat Zero = 0
fromNat (PlusOne nums) = 1 + fromNat nums
