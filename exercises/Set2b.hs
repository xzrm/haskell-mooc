...
oddFactorial n
  | val == 1 = 1
  | otherwise = val * oddFactorial (val - 2)
  where
    val = if even n then n - 1 else n
...
myGcd n k = myGcd a b
  where
    b = if k >= n then k - n else k
    a = if n > k then n - k else n
...
countdown n = "Ready! " ++ printCountdown n ++ countdown' (n - 1) ""
  where
    printCountdown n = show n ++ "... "
    countdown' n s
      | n == 0 = s ++ "Liftoff!"
      | otherwise = countdown' (n - 1) (s ++ printCountdown n)
