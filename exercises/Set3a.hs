...
import Data.List
import Data.List (foldr, length, reverse, takeWhile)
...
firstHalf :: [Char] -> [Char]
firstHalf s = take nums s
  where
    nums = if even (length s) then length s `div` 2 else (length s `div` 2) + 1

palindrome :: [Char] -> Bool
palindrome s
  | null s = False
  | length s == 1 = True
  | length s == 2 && (head s == s !! 1) = True
  | head s /= last s = False
  | otherwise = palindrome (tail (init s))
...
while check update value = if check value then while check update val else value
  where
    val = update value
...
joinToLength l lst = [i ++ j | i <- lst, j <- lst, length (i ++ j) == l]
...
xs +|+ ys
  | null xs && null ys = []
  | null xs && not (null ys) = [head ys]
  | not (null xs) && null ys = [head xs]
...
-- sumRights [] = 0
-- sumRights (x : xs) = either x + sumRights xs
--   where
--     either (Right v) = v
--     either (Left v) = 0
...
multiCompose [] v = v
multiCompose (x : xs) v = x $ multiCompose xs v
