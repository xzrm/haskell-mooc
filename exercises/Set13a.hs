...
import qualified Codec.Picture.Metadata as Map
...
import Examples.Bank (depositOp)
import Mooc.Todo
...
checkNumber (s1, s2) = if all isAlpha s1 && all isAlpha s2 then Just (s1, s2) else Nothing
...
winner scores player1 player2 = do
  p1Score <- lookup player1 scores
  p2Score <- lookup player2 scores
  if p1Score >= p2Score then Just player1 else Just player2
...
countAndLog f (x : xs)
  | f x = fmap (+ 1) $ msg (show x) >> countAndLog f xs
...
balance accountName = BankOp (\bank -> (getBalance bank, bank))
  where
    getBalance (Bank accounts) = Map.findWithDefault 0 accountName accounts
...
count x = modify (addOne x)
  where
    addOne _ [] = [(x, 1)]
    addOne x ((y, c) : ys)
      | x == y = (y, c + 1) : ys
      | otherwise = (y, c) : addOne x ys
...
occurrences xs = mapM_ count xs >> fmap length get
