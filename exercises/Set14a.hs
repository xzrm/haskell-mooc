...
greetText s = T.pack "Hello, " <> (if T.length s <= 15 then s else T.take 15 s <> T.pack "...") <> T.pack "!"
...
-- shout :: T.Text -> T.Text
shout t = T.unwords $ map (\(i, x) -> if even i then T.toUpper x else x) ws
  where
    ws = zip [0 ..] (T.words t)
...
longestRepeat t = go 0 (T.uncons t)
  where
    go len Nothing = len
    go len (Just (x, xs)) =
      let len' = T.length (T.takeWhile (== x) xs)
       in go (max (len' + 1) len) (T.uncons (T.drop len' xs))
...
takeStrict c t = TL.toStrict $ TL.take c t
...
byteRange bs = if B.length bs == 0 then 0 else B.maximum bs - B.minimum bs
...
pingpong bs =
  let lazybs = BL.fromStrict bs
   in lazybs <> BL.reverse lazybs <> pingpong bs