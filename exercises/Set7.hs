...
add i (Set xs) = if member i (Set xs) then Set xs else Set (sort (i : xs))
...
step FlourAdded AddSugar = DryMixAdded
step SugarAdded AddFlour = DryMixAdded
step DryMixAdded Mix = Mixed
...
reverseNonEmpty (x :| xs) = Data.List.NonEmpty.reverse (x :| xs)
...
instance Ord a => Semigroup (Set a) where
  Set s1 <> Set s2 = foldr add mempty (s1 ++ s2)
...
passwordAllowed str (ContainsSome (char : chars)) = elem char str || passwordAllowed str (ContainsSome chars)
passwordAllowed str (DoesNotContain []) = True
passwordAllowed str (DoesNotContain (char : chars)) = notElem char str && passwordAllowed str (DoesNotContain chars)
