...
deposit db accName amt = execute db depositQuery (accName, amt)
...
balance db accName = do
  res <- query db balanceQuery [accName] :: IO [[Int]]
...
parseCommand [command, account]
  | T.unpack command == "balance" = Just (Balance account)
  | otherwise = Nothing
parseCommand [command, account, amountText] =
  parseInt amountText >>= f
  where
    f a
      | T.unpack command == "deposit" = Just (Deposit account a)
      | T.unpack command == "withdraw" = Just (Withdraw account a)
      | otherwise = Nothing
