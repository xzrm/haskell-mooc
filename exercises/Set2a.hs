...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs = if length xs < n then xs else drop num xs
  where
    num = length xs - n
...
palindromify s = if s == reverse s then s else palindromify str
  where
    str = take (length (drop 1 s) - 1) (drop 1 s)
...
addEithers (Right i) (Left str) = Left str
addEithers (Left str1) (Left str2) = Left str1
addEithers (Left i) (Right j) = Left i
