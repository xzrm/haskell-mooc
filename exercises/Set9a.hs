...
countValid xs = length $ filter (\x -> (x !! 2 == x !! 4) || (x !! 3 == x !! 5)) xs
...
sumSuccess xs = if null vals then Left "no data" else Right (sum vals)
  where
    vals = rights xs
...
data Lock = Lock {o :: Bool, code :: String}
  deriving (Show)
...
isOpen :: Lock -> Bool
isOpen (Lock open code) = open

-- open tries to open the lock with the given code. If the code is
-- wrong, nothing happens.
open :: String -> Lock -> Lock
open pwd (Lock False code) = if code == pwd then Lock True code else Lock False code
open pwd (Lock True code) = Lock True code
...
changeCode newCode (Lock True code) = Lock True newCode
...
compose xs ys = map (\(x, Just y) -> (x, y)) $ filter (\(x, y) -> isJust y) (foldr f [] xs)
  where
    f x y = (fst x, lookup (snd x) ys) : y
