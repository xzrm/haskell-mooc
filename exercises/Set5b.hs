...
allValues condition (Node n l r) = condition n && allValues condition l && allValues condition r
...
isOrdered (Node n l r) = allValues (< n) l && allValues (> n) r && isOrdered l && isOrdered r
...
walk (step : steps) (Node n l r)
  | step == StepL = walk steps l
...
set [] val Empty = Empty
set (step : steps) val (Node n l r)
  | step == StepL = Node n (set steps val l) r
  | otherwise = Node n l (set steps val r)
...
-- search val (Node n l r) = go (Node n l r)
--   where
--     go (Node val _ _) = []
--     go Empty = Nothing
--     go (Node n l r) = StepL : go l
search = todo
