...
longest :: Ord a => [[a]] -> [a]
longest xs = minimumBy (comparing head) $ filter (\x -> length x == lenLast) sortedLst
  where
    sortedLst = sortOn length xs
    lenLast = length (last sortedLst)
...
------------------------------------------------------------------------------
-- Ex 12: given an Array, find the index of the larget element. You
