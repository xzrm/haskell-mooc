...
hSelectLines h nums = do
  lines <- hFetchLines h
  return $ (map snd . filter (\(idx, line) -> idx `elem` nums)) (zip [1 ..] lines)
...
interact' f state = do
  line <- getLine
  let (b, str, newState) = f (line, state)
  putStrLn str
  if b then interact' f newState else return newState

--   case f (line, state) of
--     (False, str, newState) -> do
--       putStrLn str
--       return newState
--     (_, str, newState) -> do
--       putStrLn str
