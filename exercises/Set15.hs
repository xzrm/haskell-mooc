...
import Examples.Validation (invalid)
import Mooc.Todo
...
statements xs ys = say <$> xs <*> [" ", " not "] <*> ys
  where
    say x i y = x ++ " is" ++ i ++ y
...
calculator op x = liftA2 f (parseOp op) (parseValue x)
  where
    f op x = op * x
    parseOp op
      | op == "negate" = Just (-1)
...
validateAddress streetName streetNumber postCode = Address <$> checkLength streetName <*> checkDigits streetNumber <*> checkPostCode postCode
...
twoPersons ::
  ...
  f Bool ->
  f [Person]
twoPersons name1 age1 employed1 name2 age2 employed2 = liftA2 combine p1 p2
  where
    combine x y = [x, y]
...
boolOrInt x = evalBool <|> evalInt
  where
    evalBool = case readMaybe x :: Maybe Bool of
      Just b -> check True "" (Left b)
      Nothing -> invalid "Not a Bool"
    evalInt = case readMaybe x :: Maybe Int of
...
normalizePhone xs = checkLength s *> checkDigits s
  where
...
parseExpression s = case words s of
  ...
  where
    ...
    parseNum a = check (all isDigit a) ("Invalid number: " ++ a) (Number (read a))
    parseVar a = check (length a == 1 && isAlpha (head a)) ("Invalid variable: " ++ a) (Variable (head a))
