...
cut (Shape f1) (Shape f2) = Shape f
  where
    f coord = f1 coord && not (f1 coord && f2 coord)
...
paintSolid color (Shape f) (Picture basef) = Picture g
  where
    g coord
      | f coord = color
...
paint (Picture p) (Shape f) (Picture g) = Picture h
  where
    h coord
      | f coord = p coord
      | otherwise = g coord
...
avgColor cs = avg (sumColors cs) len
  where
    sumColors = foldr add (Color 0 0 0)
    avg (Color r g b) len = Color (r `div` len) (g `div` len) (b `div` len)
    len = length cs
...
instance Transform Blur where
  apply Blur (Picture f) = Picture g
    where
      ...
      g (Coord x y) = avgColor (lst x y)
      lst x y = [f (Coord (x + i) (y + j)) | (i, j) <- d]
...
instance Transform BlurMany where
  ...
  apply (BlurMany n) (Picture f) = apply (BlurMany (n - 1)) p
    where
      p = apply Blur (Picture f)
