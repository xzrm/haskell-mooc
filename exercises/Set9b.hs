...
prettyPrint size coords = concat [f (r, c) | r <- [1 .. size], c <- [1 .. size]]
  where
    f (r, c)
      | elem (r, c) coords && c == size = "Q\n"
      | (r, c) `elem` coords = "Q"
      | c == size = ".\n"
      | otherwise = "."
...
danger coord = any ((== True) . f)
  where
    f :: Coord -> Bool
    f c = any (($ c) . ($ coord)) [sameRow, sameCol, sameDiag, sameAntidiag]
...
prettyPrint2 size stack = concat [f (r, c) | r <- [1 .. size], c <- [1 .. size]]
  where
    f (r, c)
      | elem (r, c) stack && c == size = "Q\n"
      | (r, c) `elem` stack = "Q"
      | danger (r, c) stack && c == size = "#\n"
      | danger (r, c) stack = "#"
      | c == size = ".\n"
      | otherwise = "."
...
-- A working but misunderstood solution; here for learning purposes
...
-- fixFirst n stack = go 0 stack
--   where
--     go i s
--       | i == length s = Just s
--       | snd (s !! i) > n = Nothing
--       | danger (s !! i) (remove i s) = go i (replaceAtIndex i (nextCol (s !! i)) s)
--       | otherwise = go (i + 1) s

fixFirst :: Size -> Stack -> Maybe Stack
fixFirst n (s : stack)
  | snd s > n = Nothing
  | danger s stack = fixFirst n (nextCol s : stack)
  | otherwise = Just (s : stack)
...
finish n stack = if length stack == (n + 1) then tail stack else finish n (step n stack)
